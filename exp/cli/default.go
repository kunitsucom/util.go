...
func (cmd *Command) loadDefaults() error {
	for _, opt := range cmd.Options {
		...
		switch o := opt.(type) {
		case *StringOption:
			DebugLog.Printf("%s: %s=%s", cmd.GetName(), o.Name, *o.Default)
			o.value = o.Default
		case *BoolOption:
			DebugLog.Printf("%s: %s=%t", cmd.GetName(), o.Name, *o.Default)
			o.value = o.Default
		case *IntOption:
			DebugLog.Printf("%s: %s=%d", cmd.GetName(), o.Name, *o.Default)
			o.value = o.Default
		case *Float64Option:
			DebugLog.Printf("%s: %s=%f", cmd.GetName(), o.Name, *o.Default)
			o.value = o.Default
		default:

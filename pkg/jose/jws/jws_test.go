...
type testCaseSign struct {
	...
	Signature     []byte
	ResultHandler func(t *testing.T, testCase testCaseSign, actual []byte, err error)
}
...
var testCasesSign = map[string]testCaseSign{
	"success(HS256)": {
		...
		ResultHandler: func(t *testing.T, testCase testCaseSign, actual []byte, err error) {
			t.Helper()
			if !bytes.Equal(testCase.Signature, actual) {
	...
	"success(ES256)": {
		...
		ResultHandler: func(t *testing.T, testCase testCaseSign, actual []byte, err error) {
			...
			token := testCase.SigningInput + "." + string(actual)
			if err := jws.Verify(token, must.One(x509z.ParseECDSAPublicKeyPEM([]byte(testz.TestECDSAPublicKey256BitPEM)))); err != nil {
	...
	"failure(ES256,jws.ErrInvalidKeyReceived)": {
		...
		SigningInput: "eyJhbGciOiJFUzI1NiIsInR5cCI6IkpXVCJ9.eyJzdWIiOiIxMjM0NTY3ODkwIiwibmFtZSI6IkpvaG4gRG9lIiwiYWRtaW4iOnRydWUsImlhdCI6MTUxNjIzOTAyMn0",
		Key:          0,
		ResultHandler: func(t *testing.T, _ testCaseSign, actual []byte, err error) {
			t.Helper()
			if !errors.Is(err, jws.ErrInvalidKeyReceived) {
	...
	"success(ES384)": {
		...
		ResultHandler: func(t *testing.T, testCase testCaseSign, actual []byte, err error) {
			...
			token := testCase.SigningInput + "." + string(actual)
			if err := jws.Verify(token, must.One(x509z.ParseECDSAPublicKeyPEM([]byte(testz.TestECDSAPublicKey384BitPEM)))); err != nil {
				t.Errorf("%s: expect(%v) != actual(%v): token: %s", t.Name(), nil, err, token)
			}
		},
	},
	...
	"success(ES512)": {
		...
		ResultHandler: func(t *testing.T, testCase testCaseSign, actual []byte, err error) {
			...
			token := testCase.SigningInput + "." + string(actual)
			if err := jws.Verify(token, must.One(x509z.ParseECDSAPublicKeyPEM([]byte(testz.TestECDSAPublicKey521BitPEM)))); err != nil {
				t.Errorf("%s: expect(%v) != actual(%v): token: %s", t.Name(), nil, err, token)
			}
		},
	},
	...
	"success(PS256)": {
		...
		Key:          must.One(x509z.ParseRSAPrivateKeyPEM([]byte(testz.TestRSAPrivateKey2048BitPEM))),
		Signature:    []byte("iOeNU4dAFFeBwNj6qdhdvm-IvDQrTa6R22lQVJVuWJxorJfeQww5Nwsra0PjaOYhAMj9jNMO5YLmud8U7iQ5gJK2zYyepeSuXhfSi8yjFZfRiSkelqSkU19I-Ja8aQBDbqXf2SAWA8mHF8VS3F08rgEaLCyv98fLLH4vSvsJGf6ueZSLKDVXz24rZRXGWtYYk_OYYTVgR1cg0BLCsuCvqZvHleImJKiWmtS0-CymMO4MMjCy_FIl6I56NqLE9C87tUVpo1mT-kbg5cHDD8I7MjCW5Iii5dethB4Vid3mZ6emKjVYgXrtkOQ-JyGMh6fnQxEFN1ft33GX2eRHluK9eg"),
		ResultHandler: func(t *testing.T, testCase testCaseSign, actual []byte, err error) {
			t.Helper()
			if err != nil {
				t.Errorf("%s: err != nil: %v", t.Name(), err)
			...
			token := testCase.SigningInput + "." + string(actual)
			if err := jws.Verify(token, must.One(x509z.ParseRSAPublicKeyPEM([]byte(testz.TestRSAPublicKey2048BitPEM)))); err != nil {
				t.Errorf("%s: expect(%v) != actual(%v): token: %s", t.Name(), nil, err, token)
			}
		},
	},

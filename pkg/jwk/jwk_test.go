...
func TestClient_GetJSONWebKey(t *testing.T) {
	...
	rootURI := fmt.Sprintf("http://%s", s.Listener.Addr().String())

	t.Run("success()", func(t *testing.T) {
		t.Parallel()

		c := jwk.NewClient(jwk.WithURLCacheClient(urlcache.NewClient[*jwk.JWKSet](http.DefaultClient)))
	...
	t.Run("failure(*json.Decoder)", func(t *testing.T) {
		...
		c := jwk.NewClient()
		_, err := c.GetJWKSet(context.Background(), rootURI)
		if err == nil {
			t.Errorf("err == nil")
		}
		expect := io.EOF.Error()
